...
%}
strData = wholeData(lineData+1:end);
for i = 1:size(strData,1)
  data(i,:)=strsplit(strData{i},',');
  sizefile = size(data,2);
  classdata=data{i,sizefile}(6:size(data{i,sizefile},2));
  data(i,sizefile)=cellstr(classdata);
end
  data=str2double(cellstr(data));